...
impl VoiceChatCryptoEngine {
    /// Initialize a new local device identity (application: initializeDevice).
    ...
    pub fn simulate_crash_reload(&mut self) -> Result<(), CryptoError> {
        ...
        {
            self.identity = IdentityKeyPair::deserialize(&blob.0).map_err(CryptoError::from)?;
        }
        if let Some(blob) = self
            .storage
            ...
            .map_err(|_| CryptoError::Storage)?
        {
            self.prekeys = PrekeyStore::deserialize(&blob.0).map_err(CryptoError::from)?;
        }
        ...
        if let Some(blob) = self
            ...
            .map_err(|_| CryptoError::Storage)?
        {
            if let Ok(cache) = ReplayCache::deserialize(&blob.0) {
                self.replay = cache;
            }
        }
        if let Some(blob) = self
            ...
            .map_err(|_| CryptoError::Storage)?
        {
            if let Ok(store) = TrustStore::deserialize(&blob.0) {
                self.trust = store;
            }
        }
...
impl CryptoEngineApi for VoiceChatCryptoEngine {
    ...
    fn encrypt_voice_payload(
        ...
        // Refuse if AD looks like it embeds a voice-profile identifier.
        // Application must never put voice-profile bytes in AD or plaintext
        // metadata — only the synthetic-voice *payload* (sender-encoded audio).
